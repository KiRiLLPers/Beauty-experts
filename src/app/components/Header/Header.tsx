...
const Header = () => {
    ...
    return (
        <header className='w-full border-b border-b-soft-light'>
            <div className='max-w-[1440px] w-full px-16 py-6 flex justify-between items-center m-auto 2xl-max:px-[40px] xl-max:px-[20px] xl-max:py-[16px] lg-max:px-[16px]'>
                <div className='flex'>
                    <ul className='flex gap-7 w-[536px] xl-max:hidden items-center'>
                        {menuTab.map(([title, url]) => (
                ...
                <div className='flex-1 ml-[40px] xl-max:ml-0 xl-max:flex xl-max:justify-center'>
                    <Image src={logo} alt='logo' className='object-cover cursor-pointer lg-max:ml-7 md-max:w-[112px]'></Image>
                </div>
